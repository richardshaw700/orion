...
public static class Wlan
{
	...
	internal struct o9dMYRVbt2EHHdbvay
	{
		public uint u0BxSTZcic;

		public uint Q8gxgY5akq;
	}
	...
	public struct WlanAvailableNetwork
	{
		...
		private uint sJdxND86gs;

		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)]
		private Dot11PhyType[] AbexcNybIk;
		...
		private uint PLNxIW06RD;

		public Dot11PhyType[] Dot11PhyTypes
		{
			get
			{
				Dot11PhyType[] array = new Dot11PhyType[sJdxND86gs];
				Array.Copy(AbexcNybIk, array, sJdxND86gs);
				return array;
			}
		}
	}
	...
	public delegate void WlanNotificationCallbackDelegate(ref WlanNotificationData notificationData, nint context);
	...
	public struct WlanConnectionParameters
	{
		...
		public nint dot11SsidPtr;

		public nint desiredBssidListPtr;
	...
	internal struct ILuWLWRJDYm9oUEJBL
	{
		internal uint t2gxqmypRq;

		internal uint TBcxPgROub;
	}
	...
	{
		private uint FBCxzh9UAb;

		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 126)]
		private ushort[] pJt6dZqsMo;

		public ushort[] Rates
		{
			get
			{
				ushort[] array = new ushort[FBCxzh9UAb / 2];
				Array.Copy(pJt6dZqsMo, array, array.Length);
				return array;
			}
		}
		...
		{
			if (rateIndex < 0 || rateIndex > pJt6dZqsMo.Length)
			{
				throw new ArgumentOutOfRangeException(so95DAFNMfktfjp4Dm.An6v7kuCGg(25130));
			}
			return (double)(pJt6dZqsMo[rateIndex] & 0x7FFF) * 0.5;
		}
	}

	public class WlanException : Exception
	{
		private readonly WlanReasonCode FeT6iPetjA;

		public WlanReasonCode ReasonCode => FeT6iPetjA;
		...
		public override string Message
		{
			get
			{
				...
				if (WlanReasonCodeToString(FeT6iPetjA, stringBuilder.Capacity, stringBuilder, IntPtr.Zero) != 0)
				{
					return string.Empty;
				}
	...
	internal struct yFrBNPAOw6BkivSbLq
	{
		public uint s5q6uDA7IU;

		public uint TlC6Vq9Kqo;
	}

	internal struct n0QCQ7kTdSKSPpInHn
	{
		public uint vH26RrDpAH;

		public uint X9t6AD2lXF;
	}
	...
	public struct WlanRadioState
	{
		...
		private WlanPhyRadioState[] kDA6kPhMlX;
		...
		public WlanPhyRadioState[] PhyRadioState
		{
			get
			{
				...
				Array.Copy(kDA6kPhMlX, array, numberofItems);
				return array;
			}
		}
	}
	...
	public static extern int WlanOpenHandle([In] uint clientVersion, [In][Out] nint pReserved, out uint negotiatedVersion, out nint clientHandle);

	[DllImport("wlanapi.dll")]
	public static extern int WlanCloseHandle([In] nint clientHandle, [In][Out] nint pReserved);

	[DllImport("wlanapi.dll")]
	public static extern int WlanEnumInterfaces([In] nint clientHandle, [In][Out] nint pReserved, out nint ppInterfaceList);

	[DllImport("wlanapi.dll")]
	public static extern int WlanQueryInterface([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] WlanIntfOpcode opCode, [In][Out] nint pReserved, out int dataSize, out nint ppData, out WlanOpcodeValueType wlanOpcodeValueType);

	[DllImport("wlanapi.dll")]
	public static extern int WlanSetInterface([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] WlanIntfOpcode opCode, [In] uint dataSize, [In] nint pData, [In][Out] nint pReserved);

	[DllImport("wlanapi.dll")]
	public static extern int WlanScan([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] nint pDot11Ssid, [In] nint pIeData, [In][Out] nint pReserved);

	[DllImport("wlanapi.dll")]
	public static extern int WlanGetAvailableNetworkList([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] WlanGetAvailableNetworkFlags flags, [In][Out] nint reservedPtr, out nint availableNetworkListPtr);

	[DllImport("wlanapi.dll")]
	public static extern int WlanSetProfile([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] WlanProfileFlags flags, [In][MarshalAs(UnmanagedType.LPWStr)] string profileXml, [Optional][In][MarshalAs(UnmanagedType.LPWStr)] string allUserProfileSecurity, [In] bool overwrite, [In] nint pReserved, out WlanReasonCode reasonCode);

	[DllImport("wlanapi.dll")]
	public static extern int WlanGetProfile([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In][MarshalAs(UnmanagedType.LPWStr)] string profileName, [In] nint pReserved, out nint profileXml, [Optional] out WlanProfileFlags flags, [Optional] out WlanAccess grantedAccess);

	[DllImport("wlanapi.dll")]
	public static extern int WlanGetProfileList([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] nint pReserved, out nint profileList);

	[DllImport("wlanapi.dll")]
	public static extern void WlanFreeMemory(nint pMemory);

	[DllImport("wlanapi.dll")]
	public static extern int WlanReasonCodeToString([In] WlanReasonCode reasonCode, [In] int bufferSize, [In][Out] StringBuilder stringBuffer, nint pReserved);

	[DllImport("wlanapi.dll")]
	public static extern int WlanRegisterNotification([In] nint clientHandle, [In] WlanNotificationSource notifSource, [In] bool ignoreDuplicate, [In] WlanNotificationCallbackDelegate funcCallback, [In] nint callbackContext, [In] nint reserved, out WlanNotificationSource prevNotifSource);

	[DllImport("wlanapi.dll")]
	public static extern int WlanConnect([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] ref WlanConnectionParameters connectionParameters, nint pReserved);

	[DllImport("wlanapi.dll")]
	public static extern int WlanDeleteProfile([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In][MarshalAs(UnmanagedType.LPWStr)] string profileName, nint reservedPtr);

	[DllImport("wlanapi.dll")]
	public static extern int WlanGetNetworkBssList([In] nint clientHandle, [In][MarshalAs(UnmanagedType.LPStruct)] Guid interfaceGuid, [In] nint dot11SsidInt, [In] Dot11BssType dot11BssType, [In] bool securityEnabled, nint reservedPtr, out nint wlanBssList);

	[DebuggerStepThrough]
	internal static void AcQxMlYEKl(int P_0)
	{
		if (P_0 != 0)
		{
